test_that("fine scale, strata, and enviro is working",
	{
		skip_on_travis()
  		...
  		# fit model
  			vast_output = fit.vast(Data_Geostat,RunDir=getwd(),SaveDir=getwd(),save.output=FALSE,Q_ik = NULL,vf.re = FALSE,FieldConfig=c(Omega1 = 1, Epsilon1 = 1, Omega2 = 1, Epsilon2 = 1),RhoConfig=c(Beta1 = 0, Beta2 = 0, Epsilon1 = 0, Epsilon2 = 0),ObsModel_ez = c(1,3),fine_scale=TRUE,input.grid.res=1,knot_method = "grid",n_x=100,Version="VAST_v8_3_0",Method="Mesh",strata.sp=skj.alt2019.shp,enviro=enviro)
  			# Par1 = fse.par
		...
		expect_lt(vast_output$Opt$max_gradient,expected=1e-4)
	})
